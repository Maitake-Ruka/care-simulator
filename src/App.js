...
import { AlertCircle, Heart, TrendingUp, Calculator, Clock } from 'lucide-react';
...
const CareCostSimulator = () => {
  ...
  const calculate = () => {
    ...
    let fatherTimeHours = 0, motherTimeHours = 0;
    ...
    if (hasFather) {
      ...
      fatherTimeHours = 5 * 365 * fatherTotalYears;
    }
    ...
    if (hasMother) {
      ...
      motherTimeHours = 5 * 365 * motherTotalYears;
    }
    ...
    const totalTimeHours = fatherTimeHours + motherTimeHours;
    const totalTimeDays = Math.round(totalTimeHours / 24);
    const totalTimeYears = (totalTimeHours / (24 * 365)).toFixed(1);
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          {result && (
            <div className="bg-gradient-to-br from-orange-50 to-amber-50 p-8 border-t-4 border-orange-500">
              ...
              <div className="bg-gradient-to-r from-orange-500 to-amber-500 p-8 rounded-2xl shadow-lg text-white text-center mb-6">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-md">
                ...
                {result.hasFather && (
                  <div className="mb-4 bg-blue-50 p-4 rounded-xl">
                    <p className="font-bold text-blue-800 mb-2">【父親の介護時間】</p>
                    <p className="text-gray-700">
                      ・1日5時間 × 365日 × {result.fatherTotalYears}年間 = 
                      <span className="font-bold text-blue-600"> {result.fatherTimeHours.toLocaleString()}時間</span>
                    </p>
                    <p className="text-sm text-gray-600 mt-1">
                      （約{Math.round(result.fatherTimeHours / 24).toLocaleString()}日分 / 約{(result.fatherTimeHours / (24 * 365)).toFixed(1)}年分）
                    </p>
                  </div>
                )}

                {result.hasMother && (
                  <div className="mb-4 bg-pink-50 p-4 rounded-xl">
                    <p className="font-bold text-pink-800 mb-2">【母親の介護時間】</p>
                    <p className="text-gray-700">
                      ・1日5時間 × 365日 × {result.motherTotalYears}年間 = 
                      <span className="font-bold text-pink-600"> {result.motherTimeHours.toLocaleString()}時間</span>
                    </p>
                    <p className="text-sm text-gray-600 mt-1">
                      （約{Math.round(result.motherTimeHours / 24).toLocaleString()}日分 / 約{(result.motherTimeHours / (24 * 365)).toFixed(1)}年分）
                    </p>
                  </div>
                )}
                ...
                <div className="bg-gradient-to-r from-green-500 to-emerald-500 p-6 rounded-xl text-white text-center">
                  ...
                  <p className="text-sm opacity-90 mt-2">
                    （約{result.totalTimeDays.toLocaleString()}日分）
                  </p>
                </div>
              </div>
            </div>
